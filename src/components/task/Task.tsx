...
const Task: React.FC<TaskProps & TaskToptions> = ({
  ...
  return (
      <div className={`gogo__board__task ${task.complete ? "task-completed" : "" } ${theme.themeMode  === "dark"? "gogo__task__at__dark__mode": ""}`}>
        <div className={`gogo__check_box`}>
          <label htmlFor={`check_box_${taskId}`}>{task.complete ? "not-completed?": "completed ?"}</label>
          <input type="checkbox" onChange={() => onCheckOrUnCheck(task.complete, task.taskId, boardId)} checked={task.complete}
          />
        </div>
              ...
              <div className="gogo__task__options__body">
                ...
                <div
                    className={`gogo__task__title__update ${
                        isUserWantToChangeTheTaskTitle ? "d-yes-title" : "d-no-title"
                    }`}
                >
                  <Input
                ...
                <div
                    className={`gogo__task__desc__update  ${
                        isUserWantToChangeTheTaskDesc ? "d-yes-desc" : "d-no-desc"
                    }`}
                >
                  <Input
