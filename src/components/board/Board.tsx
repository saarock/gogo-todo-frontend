...
const Board: React.FC<BoardProps> = ({
    ...
    const updateTaskTitle = useCallback(
        async (taskId: number, boardId: number) => {
            try {
                setLoading(true)
                ...
                toast.error(
                    error instanceof Error
                        ? error.message
                        : 'Something wrong while updating the task details'
                )
            } finally {
                setTaskOption(false)
                setLoading(false)
    ...
    const deleteTask = useCallback(
        async (boardId: number) => {
            try {
                ...
                if (boardId <= -1)
                    throw new Error(
                        'Invalid board id pleased referesh the page and try again'
                    )
                ...
                toast.error(
                    error instanceof Error
                        ? error.message
                        : 'Cannot delete  the task pleased refresh and try again'
                )
            } finally {
                setLoading(false)
                setTaskOption(false)
    ...
    const clickForUpdateTaskTitle = useCallback(() => {
        ...
        taskState.isUserWantToUpdateTheTaskTitle,
        taskState.isUserWantToUpdateTheTaskDesc,
    ])
    ...
    const onChangeNewTaskTitle = useCallback(
    ...
    const onChangeTaskNewDesc = useCallback(
        (e: React.ChangeEvent<HTMLInputElement>) => {
            taskDispatch({
                type: TaskActionTypes.SET_TASK_DESC,
                payload: e.target.value,
    ...
    const handelCheckTask = useCallback(
        async (is: boolean, taskId: number, boardId: number) => {
            try {
                ...
                toast.error(
                    error instanceof Error
                        ? error.message
                        : 'Something wrong pleased try again by reloading the page'
                )
            }
        },
        []
    ...
    return (
        ...
        >
            {whichBoardOptionsUserWantToSee == board.boardId &&
                isWantToSeeOptions && (
                    <div className="gogo__options__of__board">
                        <Button
                            onClick={() => updateBoardName(board.boardId || -1)}
                            text="Update"
                            icon={<FaEdit />}
                        />
                        <Button
                            text="Delete"
                            onClick={() => deleteBoard(board.boardId || -1)}
                            icon={<MdDelete />}
                        />
            ...
            <span
                onClick={() => openBoardOptions(board.boardId || -1)}
                ...
                <div>
                        ...
                        <h2>
                            ...
                            board.boardId === whichBoardOptionsUserWantToSee ? (
                                <div className={`gogo__board_update__buttons`}>
                                    <div className={`gogo__board__input`}>
                                        <Input
                                            type="text"
                                            onChange={onChangeEventOfBoardName}
                                        />
                                    </div>
                                    <div className={`gogo__board__buttons`}>
                                        <Button
                                            onClick={saveNewBoardName}
                                            text="Saved"
                                            className="gogo__board__newname__saved__button gogo__button"
                                        />
                                        <Button
                                            onClick={cancleUpdateBoardName}
                                            text="Cancel"
                                            className="gogo__board__delete__button gogo__button"
                                        />
                                    </div>
                                </div>
                            ) : (
                    ...
                    {board.tasks.length >= 1 &&
                        board.tasks.map((task) => (
                            <div
                                className="gogo__task__container"
                                key={task.taskId}
                            >
                                <Task
                                    task={task || 'Loading...'}
                    ...
                    {onWhichBoardUserWantToAddTheTask === board.name && (
                        <div className="gogo__board__title">
                            ...
                            <div className={`gogo__buttons__for__task`}>
                                <Button
                                    text="Add"
                                    className={`gogo__button`}
                                    onClick={() =>
                                        createNewTask(
                                            board.boardIndex || 0,
                                ...
                                <Button
                                    onClick={onClickListenerToCancelTheAddTask}
                                    text="cancle"
                                    className={`gogo__button`}
                                />
                            </div>
