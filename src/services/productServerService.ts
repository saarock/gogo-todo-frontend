...
class ProductServerService {
    ...
    public async createNewBoard(board: Board): Promise<void> {
        try {
            const response = await axiosInstance1.post('/create-board', {
                ...board,
            })
            const data = await response.data
            if (data.status !== 'OK')
    ...
    public async createNewTask(task: Task) {
        try {
            const response = await axiosInstance1.post('/create-task', {
                ...task,
            })
            const data = await response.data
            if (data.status !== 'OK') throw new Error(data.message)
            ...
            if (axios.isAxiosError(error)) {
                if (error.response) {
                    const isUnauthorized =
                        responseServerUtil.checkIsUnauthorized(
    ...
    public async updateProduct(
        productUpdateDetails: ProductNameandId
    ): Promise<void> {
        try {
    ...
    public async checkCompleteOrNotCompleteBoard(
        boardId: number,
        isCompleted: boolean
    ): Promise<boolean> {
        try {
            const response = await axiosInstance1.put(
                `/check-complete-or-not/${boardId}`,
                { isCompleted }
            )
            const data = await response.data
            if (data.status !== 'OK') throw new Error(data.message)
    ...
    public async checkCompleteOrNotCompleteTask(
        id: number,
        isCompleteTask: boolean
    ): Promise<boolean> {
        try {
            const response = await axiosInstance1.put(
                `/check-complete-or-not-task/${id}`,
